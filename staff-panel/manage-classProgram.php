    ...
    <div class="wrapper">
        ...
        <form action="" method="POST" class="dropdown-form">
            <select name="file-select" class="styled-select" onchange="location = this.value;">
                ...
                <option value="manage-classProgram.php" selected>Manage Classes</option>
                <option value="manage-training.php">Manage Training</option>
                <option value="manage-nutration.php">Manage Nutrition</option>
            </select>
        </form>
        ...
        <div class="programs-grid">
            <?php 
            // Fetch classes from tbl_programs where category is related to classes
            $sql = "SELECT * FROM tbl_programs WHERE category_id = (SELECT id FROM tbl_programCategories WHERE title = 'Group Classes')";
            $res = $conn->query($sql);
            ...
            if ($res->num_rows > 0) {
                while ($row = $res->fetch_assoc()) {
                    $program_id = $row['id'];
                    $title = $row['title'];
                    $price_local = $row['price_local']; // Price for locals
                    $price_foreign = $row['price_foreign']; // Price for foreigners
...
<p><span class="label">Time Schedule:</span></p>
                        ...
                        <!-- <div class="program-actions">
                            <a href="update-classProgram.php?id=<?php echo $program_id; ?>" class="btn-secondary">Update</a>
